        ...
        if os.path.exists(CONFIG_FILE):
            with open(CONFIG_FILE, "r") as f:
                try:
                    ...
                    if (existing_config.get("partner_name") == partner_name and 
                        existing_config.get("partner_ip") == partner_ip):
...
def main():
    ...
    if config:
        ...
        if use_config == "y":
            ...
            print(f"Using saved configuration: Partner Name: {partner_name}, IP: {partner_ip}")
        else:
    ...
    connector = threading.Thread(target=client_thread_func, args=(partner_ip,), daemon=True)
    listener.start()
    connector.start()

    # Wait up to 60 seconds for a successful connection.
    if not connection_established.wait(timeout=60):
        print("Unable to establish connection. Please check the partner's IP address and try again.")
        return
